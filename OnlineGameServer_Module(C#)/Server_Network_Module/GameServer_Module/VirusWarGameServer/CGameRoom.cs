...
namespace VirusWarGameServer
{
    ...
    class CGameRoom
    {
        ...
        readonly short EMPRY_SLOT = short.MaxValue;
        ...
        // 모든 플레에이가 같은 상태에 있다면 true, 한명이라도 다른 상태에 있다면 false를 반환
        ...
        public void enter_gameroom(CGameUser user1, CGameUser user2)
        {
            ...
            CPlayer player2 = new CPlayer(user2, 0);
        ...
        void reset_gamedata()
        {
            ...
            {
                this.gameboard[i] = EMPRY_SLOT;
            }

            // 1번 플레이어의 세균은 왼쪽 위(0,0), 오른쪽 위(0, 6) 두군데에 배치한다.
            put_virus(0, 0, 0);
            put_virus(0, 0, 6);

            // 2번 플레이어의 세균은 왼쪽 아래(6, 0), 오른쪽 아래(6,6) 두군데에 배치한다.
            put_virus(1, 6, 0);
            put_virus(1, 6, 0);
        ...
        {
            this.gameboard[position] = EMPRY_SLOT;
        ...
        {
            // 방어자의 세균중에 기존위치로 부터 1칸 반경에 있는 세균들이 감염 대상이다.
        ...
        public void moving_req(CPlayer sender, short begin_pos, short target_pos)
        {
            ...
            if(this.gameboard[target_pos] != EMPRY_SLOT)
            {
                // 목적지에 다른 세균이 존재한다.
                return;
            }
            ...
            // 목적지를 기준으로 주위에 존재하는 상배당 세균을 감염시켜 같은 편으로 마든다.
            CPlayer opponent = get_opponent_player();
            infect(target_pos, sender, opponent);

            // 최종 결과를 broadcast한다.

            CPacket msg = CPacket.create((short)PROTOCOL.PLYAER_MOVED);
        ...
        // 클라이언트에서 턴 연출이 모두 완료 되었을 때 호출되낟.
