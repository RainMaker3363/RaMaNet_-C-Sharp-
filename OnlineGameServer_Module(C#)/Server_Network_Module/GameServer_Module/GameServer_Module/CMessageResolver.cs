...
namespace GameServer_Module
{
    ...
    // - header : 데이터 사이즈. Define.HEADERSIZE에 정의된 타입만큼의 크기를 갖는다.
    //            2바이트일 경우 Int16, 4바이트는 Int32로 처리하면 된다.
    //             본문의 크기가 Int16.Max값을 넘지 않는다면 2바이트로 처리하는것이 좋다
    // - body : 메시지 본문.
    class CMessageResolver
    {
        ...
        // 진행중인 버퍼.
        byte[] message_buffer = new byte[1204];

        // 현재 진행중인 버퍼의 인덱스를 가리키는 변수
        // 패킷 하나를 완성한 뒤에는 0으로 초기화 시켜줘야 하낟.
