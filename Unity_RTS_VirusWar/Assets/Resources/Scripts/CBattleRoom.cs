...
public class CBattleRoom : MonoBehaviour {
    ...
    // 게임 종료 후 메인으로 돌아갈 때 사용하기 위한 Maintitle의 객체의 래퍼런스
    CMainTitle main_title;

    // 네트워크 데이터 송, 수신을 위한 네트워크 매니저 래퍼런스
    CNetworkManager network_manager;

    // 게임 상태에 따른 각각 다른 GUI모습을 구현하기 위해 필요한 상태 변수.
    ...
    {
        byte plyaer_index = msg.pop_byte();
        short from = msg.pop_int16();
        short to = msg.pop_int16();

        StartCoroutine(on_selected_cell_to_attack(plyaer_index, from, to));
    }
